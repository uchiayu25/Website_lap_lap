...
<script>
    ...
    function edit(key){
        location.replace("edit_dusun.php?key="+key)
    }
    ...
    //     $.ajax({
    //     type: 'GET',
    //     url: "datasignup.php?aksi=del_dusun&id_dusun="+key+"&id_desa="+kode,
    //     success: function(data) {
    //         dataFull = JSON.parse(data)
    //         count = 1
    //         $("#lapdusun").empty()
    ...
    //                                     "<td scope='col'>"+dataFull[key]['nama_desa']+"</td>"+
    //                                     "<td scope='col'>"+dataFull[key]['nama_dusun']+"</td>"+
    //                                     "<td class='d-inline-flex gap-3 align-items-center justify-content-center'><button type='button' class='btn btn-danger' onclick='edit("+dataFull[key]['kode_dusun']+")'><i class='fas fa-edit'></i>Hapus</button></td></tr>");
    //             count++
    //         })
    //         localStorage.setItem("DataDusun", JSON.stringify(dataFull));
