...
<script>
   datafl = JSON.parse(localStorage.getItem('dataUser'))
    ...
    function verifikasi(key){
        location.replace("verifikasi_laporan.php?key="+key)
    }
    
    $.ajax({
        type: 'GET',
        url: "datasignup.php?aksi=get_lapo5&id_desa="+datafl['kode_desa'],
        success: function(data) {
            dataFull = JSON.parse(data)
            count = 1
            $("#lapotabel").empty()
            Object.keys(dataFull).forEach(function(key){
                $("#lapotabel").append("<tr>"+
                                        ...
                                        "<td class='d-inline-flex gap-3 align-items-center justify-content-center'> <button type='button' class='btn btn-primary' onclick='verifikasi("+key+")'><i class='fas fa-edit'></i>Verifikasi</button></td></tr>");
                count++
            })
            localStorage.setItem("DataLapdesabaru", JSON.stringify(dataFull));
        }
    });
    ...
    function datachange(){
        ...
        $.ajax({
            type: 'GET',
            url: "datasignup.php?aksi=get_lapo6&kecamatan="+kecamatan+"&desa="+desa+
