<?php
    ...
    if(isset($_GET['aksi'])){
        ...
        else if($_GET['aksi']=='del_dusun'){deletedusun();}
        ...
        else if($_GET['aksi']=='get_lapokabupaten2'){viewlapkabupaten2();}
    ...
    if ( !empty($_POST['action']) ) {
        ...
        if ($_POST['action'] == "update_dusun") return updatedusun();
    ...
    function upload_file($directory,$fileToUpload){
        $target_dir = $directory;
        $file_baru = uniqid().".pdf";
        $target_file = $target_dir.$file_baru;
        $uploadOk = 1;
    ...
    function deletedusun(){
        global $conn;
        $id = $_GET['id_dusun'];
        $sql = "DELETE FROM `dusun` WHERE `kode_dusun`='$id'";
        mysqli_query($conn, $sql);
        echo $sql;
    ...
    function updatelapdesa(){
        ...
        $sql = "UPDATE `laporan` SET `status` = 'Ditolak', `komentar`='$komentar' WHERE `id_laporan`='$id'";
        echo $sql;
        mysqli_query($conn, $sql);
    }
    ...
    function viewlap2(){
        ...
        $sql = "SELECT * FROM `lap_terverikasi`".$tanggal.$search.$kecamatan.$desa.$dusun;
        $readTable = mysqli_query($conn, $sql);
        $rows = array();
        while ($getTableData = mysqli_fetch_assoc( $readTable )) $rows[]=$getTableData;
        echo json_encode($rows);
    ...
    function viewlaptolak2(){
        ...
        $sql = "SELECT * FROM `lap_ditolak`".$tanggal.$search.$kecamatan.$desa.$dusun;
        $readTable = mysqli_query($conn, $sql);
        $rows = array();
        while ($getTableData = mysqli_fetch_assoc( $readTable )) $rows[]=$getTableData;
        echo json_encode($rows);
    ...
    function viewlapobaru2(){
        ...
        $sql = "SELECT * FROM `lap_baru`".$tanggal.$search.$kecamatan.$desa.$dusun;
        $readTable = mysqli_query($conn, $sql);
        $rows = array();
        while ($getTableData = mysqli_fetch_assoc( $readTable )) $rows[]=$getTableData;
        echo json_encode($rows);
    ...
    function viewlapkabupaten2(){
        global $conn;
        $sql = "SELECT lap.*, kec.nama_kecamatan, des.nama_desa, dus.nama_dusun FROM `laporan` AS lap LEFT JOIN kecamatan AS kec USING (kode_kecamatan)
        LEFT JOIN desa AS des USING (kode_desa) LEFT JOIN dusun AS dus USING (kode_dusun) WHERE lap.status = 'Sudah Terverifikasi' ORDER BY lap.date_lapor";
        $readTable = mysqli_query($conn, $sql);
        $rows = array();
        while ($getTableData = mysqli_fetch_assoc( $readTable )) $rows[]=$getTableData;
        echo json_encode($rows);
    ...
    function tambahlap(){
        ...
        $no_kk                  = $_POST['no_kk'];
        $ktp                    = upload_file("file/ktp/","ktp_input");
        // $kk                     = upload_file("file/kartu_keluarga/","kk_input");
        // $skk                    = upload_file("file/skk/","skk_input");
        // $berkas                 = upload_file("file/berkas/","berkas_input");
        if(!$ktp){
            echo "1";
        }

        $sql = "INSERT INTO `laporan`(`date_lapor`, `kode_dusun`, `kode_desa`, `kode_kecamatan`, `NIK_m`, `nama_m`, `tempat_m`, `tanggal_m`, `tanggal_p`, `NIK_p`, `nama_p`, `hub_p`,`nama_lengkap_kepdusun`,`jenis_kelamin`,`status`,`alamat`,`no_hp`,`no_anak`,`no_kk`,`waktu_meninggal`,`ktp`)
                VALUES ('$tanggal_lapor','$dusun','$desa','$kecamatan','$NIK_m','$nama_lengkap_m','$tempat','$date_m', '$date_p', '$NIK_p', '$nama_lengkap_p', '$hub_p', '$nama_lengkap_kepdusun', '$jenis_kelamin', 'Belum Terverifikasi','$alamat','$no_hp','$no_anak','$no_kk','$waktu','$ktp')";
        echo $sql;
        mysqli_query($conn, $sql);
    }
    ...
    function updatelap(){
        ...
        $sql = "UPDATE `laporan` SET `date_lapor` = '$tanggal_lapor', `kode_dusun`= '$dusun', `kode_desa`='$desa', `kode_kecamatan`='$kecamatan', 
        `NIK_m`='$NIK_m', `nama_m`='$nama_lengkap_m', `tempat_m`='$tempat', `tanggal_m`='$date_m', 
        `tanggal_p`='$date_p', `NIK_p`='$NIK_p', `nama_p`='$nama_lengkap_p', `hub_p`='$hub_p',`jenis_kelamin`='$jenis_kelamin', 
        `alamat`='$alamat', `no_hp`='$no_hp', `status`='Belum Terverifikasi', `no_kk`='$no_kk', `no_anak`='$no_anak', `ktp`='$ktp', `waktu_meninggal` = '$waktu' WHERE `id_laporan`='$id'";
        echo $sql;
        mysqli_query($conn, $sql);
    }

    function updatedusun(){
        global $conn;
        $id                     = $_POST['dusun'];
        $nama                   = $_POST['nama'];

        $sql = "UPDATE `dusun` SET `nama_dusun` = '$nama' WHERE `kode_dusun`='$id'";
        echo $sql;
        mysqli_query($conn, $sql);
    }
    ...
    function updatelaptolak(){
        ...
        $sql = "UPDATE `laporan` SET `date_lapor` = '$tanggal_lapor', `kode_dusun`= '$dusun', `kode_desa`='$desa', 
        `kode_kecamatan`='$kecamatan', `NIK_m`='$NIK_m', `nama_m`='$nama_lengkap_m', `tempat_m`='$tempat', `tanggal_m`='$date_m', 
        `tanggal_p`='$date_p', `NIK_p`='$NIK_p', `nama_p`='$nama_lengkap_p', `hub_p`='$hub_p',`jenis_kelamin`='$jenis_kelamin', 
        `alamat`='$alamat', `no_hp`='$no_hp', `status`='Belum Terverifikasi', `no_anak`='$no_anak', `no_kk`='$no_kk', `ktp`='$ktp',`waktu_meninggal` = '$waktu', `komentar`='$komentar'
        WHERE `id_laporan`='$id'";
        echo $sql;
        mysqli_query($conn, $sql);
    }
    ...
    function listdusun3($key){
        global $conn;
        echo($key);
        $sql = "SELECT dus.*, kec.nama_kecamatan, des.nama_desa FROM `dusun` AS dus LEFT JOIN desa AS des USING (kode_desa) LEFT JOIN kecamatan AS kec USING (kode_kecamatan) WHERE dus.`kode_desa` = $key ORDER BY nama_dusun";
        $readTable = mysqli_query($conn, $sql);
        $rows = array();
        while ($getTableData = mysqli_fetch_assoc( $readTable )) $rows[]=$getTableData;
        echo json_encode($rows);
