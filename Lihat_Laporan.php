        ...
        <div class="col-7 content">
            <div class="form-row mt-lg-5">
                <div class="form-group">
                    ...
                    <select class="form-select changedata" aria-label="Default select example" id="dusun" disabled></select>
                </div>
    ...
    <div style="width:29.7cm; border: black 0.5px solid; display:none" id="exportData" >
        ...
        <div style="position:relative">
            <div style="position:absolute;right:30px">
                ...
                <p style="line-height:0px" id="jabatan_dus">Kelian Banjar/Kepala Lingkungan</p>
...
<script>
    ...
    if(datafl['role'] == "1"){
        ...
        $("#wilayahsur").html(datafl['nama_dusun'].toUpperCase());
        $("#tmptgl_petugas").html(datafl['nama_dusun']+", ")
        nama_desa_pimpin = JSON.stringify(datafl['nama_dusun']);
        resultpim = nama_desa_pimpin.includes("Lingkungan")
        if(resultpim){
            $("#jabatan_dus").html("Kepala "+datafl['nama_dusun'])
        }else{
            $("#jabatan_dus").html("Kelian "+datafl['nama_dusun'])
        }
    }
    ...
    function datachange(){
        ...
        $.ajax({
            type: 'GET',
            url: "datasignup.php?aksi=get_lapo2&kecamatan="+kecamatan+"&desa="+desa+
            ...
            success: function(data) {
                ...
                Object.keys(dataFull).forEach(function(key){
                    $("#laptabel").append("<tr>"+
                                            ...
                                            "<td scope='col'>"+dataFull[key]['status']+"</td>"+
                                            "<td class='d-inline-flex gap-3 align-items-center justify-content-center'><button type='button' class='btn btn-success' onclick='edit("+key+")'><i class='fas fa-edit'></i>Edit</button></td></tr>");
                                            // "<button type='button' class='btn btn-danger' onclick='hapus("+dataFull[key]['id_laporan']+","+dataFull[key]['kode_dusun']+")'><i class='far fa-trash-alt'></i>Hapus</button></td></tr>");
                            $("#isiTabel").append("<tr>"+
                ...
                localStorage.setItem("DataLap", JSON.stringify(dataFull));
            }
        });
    }

    function hapus(key,kode){
        $.ajax({
        type: 'GET',
        url: "datasignup.php?aksi=del_lapo&id_dusun="+kode+"&id_laporan="+key,
        success: function(data) {
            dataFull = JSON.parse(data)
            count = 1
            $("#laptabel").empty()
    ...
    function edit(key){
        location.replace("edit_laporan_dusun.php?key="+key)
    }
    ...
    $.ajax({
        type: 'GET',
        url: "datasignup.php?aksi=get_lapo&id_dusun="+datafl['kode_dusun'],
        success: function(data) {
            dataFull = JSON.parse(data)
            count = 1
            $("#laptabel").empty()
